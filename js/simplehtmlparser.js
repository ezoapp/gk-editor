...
SimpleHtmlParser.prototype = {
	...
	// endTagRe:	/^<\/([^>\s]+)[^>]*>/m,
	// attrRe:		/([^=\s]+)(\s*=\s*((\"([^"]*)\")|(\'([^']*)\')|[^>\s]+))?/gm,
	startTagRe:	/^<([-A-Za-z0-9_]+)((?:\s+[\w-]+(?:\s*=\s*(?:(?:"[^"]*")|(?:'[^']*')|[^>\s]+))?)*)\s*(\/?)>/,
	endTagRe:	/^<\/([-A-Za-z0-9_]+)[^>]*>/,
	...
	parse:	function (s, oHandler)
	{
		...
		while (s.length > 0)
		{
			...
			if (treatAsChars)
			{
				...
				{
					for (var j = s.length; index < j; index++) {
						t = s.substring(index);
						if (this.startTagRe.test(t) || this.endTagRe.test(t) || t.substring(0, 4) == "<!--") {
							break;
						}
					}
